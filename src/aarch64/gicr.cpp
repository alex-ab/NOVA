...
void Gicr::mmap_mmio()
{
    ...
    // Map MMIO region
    Hptp::current().update (MMAP_CPU_GICR, Cpu::gicr, bit_scan_msb (mmio_size) - PAGE_BITS, Paging::Permissions (Paging::G | Paging::W | Paging::R), Memattr::dev());

    // Reserve MMIO region
    Space_hst::access_ctrl (Cpu::gicr, read (Reg64::TYPER) & BIT (1) ? 2 * mmio_size : mmio_size, Paging::NONE);
}
