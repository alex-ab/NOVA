...
uint64_t Cpu::feat_cpu64[3], Cpu::feat_dbg64[3], Cpu::feat_isa64[4], Cpu::feat_mem64[5], Cpu::feat_sme64[1], Cpu::feat_sve64[1];
...
void Cpu::enumerate_features()
{
    ...
    asm volatile ("mrs %x0, ctr_el0"            : "=r" (ctr));              // 2.23
    asm volatile ("mrs %x0, midr_el1"           : "=r" (midr));             // 2.23
    asm volatile ("mrs %x0, mpidr_el1"          : "=r" (mpidr));            // 2.23
    ...
    asm volatile ("mrs %x0, id_aa64isar0_el1"   : "=r" (feat_isa64[0]));    // 2.23
    asm volatile ("mrs %x0, id_aa64isar1_el1"   : "=r" (feat_isa64[1]));    // 2.23
    asm volatile ("mrs %x0, id_aa64isar2_el1"   : "=r" (feat_isa64[2]));    // 2.38
    asm volatile ("mrs %x0, id_aa64isar3_el1"   : "=r" (feat_isa64[3]));    // 2.43
    asm volatile ("mrs %x0, id_aa64mmfr0_el1"   : "=r" (feat_mem64[0]));    // 2.23
    asm volatile ("mrs %x0, id_aa64mmfr1_el1"   : "=r" (feat_mem64[1]));    // 2.23
    ...
    if (feature (Cpu_feature::EL1) == 2) {
        ...
        asm volatile ("mrs %x0, id_isar1_el1"   : "=r" (feat_isa32[1]));    // 2.23
        asm volatile ("mrs %x0, id_isar2_el1"   : "=r" (feat_isa32[2]));    // 2.23
        asm volatile ("mrs %x0, id_isar3_el1"   : "=r" (feat_isa32[3]));    // 2.23
        asm volatile ("mrs %x0, id_isar4_el1"   : "=r" (feat_isa32[4]));    // 2.23
        asm volatile ("mrs %x0, id_isar5_el1"   : "=r" (feat_isa32[5]));    // 2.23
        asm volatile ("mrs %x0, id_isar6_el1"   : "=r" (feat_isa32[6]));    // 2.38
        asm volatile ("mrs %x0, id_mmfr0_el1"   : "=r" (feat_mem32[0]));    // 2.23
        asm volatile ("mrs %x0, id_mmfr1_el1"   : "=r" (feat_mem32[1]));    // 2.23
        asm volatile ("mrs %x0, id_mmfr2_el1"   : "=r" (feat_mem32[2]));    // 2.23
        asm volatile ("mrs %x0, id_mmfr3_el1"   : "=r" (feat_mem32[3]));    // 2.23
        asm volatile ("mrs %x0, id_mmfr4_el1"   : "=r" (feat_mem32[4]));    // 2.26
        asm volatile ("mrs %x0, id_mmfr5_el1"   : "=r" (feat_mem32[5]));    // 2.38
    }

    asm volatile ("mrs %x0, mvfr0_el1"          : "=r" (feat_mfp32[0]));    // 2.23
    asm volatile ("mrs %x0, mvfr1_el1"          : "=r" (feat_mfp32[1]));    // 2.23
    asm volatile ("mrs %x0, mvfr2_el1"          : "=r" (feat_mfp32[2]));    // 2.23
    ...
    switch (midr >> 24 & BIT_RANGE (7, 0)) {
        ...
        case 0x41:
            ...
            switch (midr >> 4 & BIT_RANGE (11, 0)) {
                ...
                case 0xd83: part = "Neoverse V3AE (Poseidon-AE)";   break;
                case 0xd84: part = "Neoverse V3 (Poseidon)";        break;
                case 0xd85: part = "Cortex-X925 (Blackhawk)";       break;
                case 0xd87: part = "Cortex-A725 (Chaberton)";       break;
                case 0xd88: part = "Cortex-A520AE (Hayes-AE)";      break;
                case 0xd89: part = "Cortex-A720AE (Hunter-AE)";     break;
                case 0xd8a: part = "C1-Nano (Nevis)";               break;
                case 0xd8b: part = "C1-Pro (Gelas)";                break;
                case 0xd8c: part = "C1-Ultra (Travis)";             break;
                case 0xd8e: part = "Neoverse N3 (Hermes)";          break;
                case 0xd8f: part = "Cortex-A320 (Arcadia)";         break;
                case 0xd90: part = "C1-Premium (Alto)";             break;
                case 0xd94: part = "Neoverse N4 (Dionysus)";        break;
                case 0xd96: part = "Canyon";                        break;
                case 0xd98: part = "Venom";                         break;
                case 0xd9a: part = "Veymont";                       break;
                case 0xda0: part = "Caddo";                         break;
                case 0xda1: part = "Rosillo";                       break;
            }
            break;
...
void Cpu::allocate (cpu_t cpu, uint64_t m, uint64_t r)
{
    ...
    hptp.update (MMAP_CPU_DSTB, Kmem::ptr_to_phys (d), 0, Paging::Permissions (Paging::G | Paging::W | Paging::R), Memattr::ram());
    ...
    // Prefill addresses
    *Kmem::loc_to_glb (cpu, &mpidr) = m;
    *Kmem::loc_to_glb (cpu, &gicr)  = r;
    *Kmem::loc_to_glb (cpu, &ptab)  = hptp.root_addr();
}
