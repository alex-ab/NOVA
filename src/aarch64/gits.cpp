...
Gits::Gits (uint64_t p, uint32_t i) : List { list }, Mmio { p, 0x10000 }, id { i }, typer { read (Reg64::TYPER) }
{
    ...
    trace (TRACE_INTR, "GITS: %#010lx %03x:%03x r%up%u v%u DEV:%u EVT:%u HCC:%u VLPI:%u PLPI:%u C:%u (#%u)",
           phys, BIT_RANGE (11, 0) & iidr, iidr >> 24, BIT_RANGE (3, 0) & iidr >> 16, BIT_RANGE (3, 0) & iidr >> 12, arch,
           ord_dev(), ord_evt(), num_hcc(), feat_vlpi(), feat_plpi(), coherent, id);
}
...
{
    // Determine order of entries that fit into the ITT
    auto const ord_itt { static_cast<unsigned>(bit_scan_msb (PAGE_SIZE (0) / esz_itt())) };

    // Fail if DID exceeds number of implemented DevID bits
    if (did >= BIT (ord_dev())) [[unlikely]]
        return Status::BAD_PAR;

    // Fail if EID exceeds number of implemented EvtID bits
    if (eid >= BIT (ord_evt())) [[unlikely]]
        return Status::BAD_PAR;

    // Fail if EID exceeds number of allocated ITT entries
    if (eid >= BIT (ord_itt)) [[unlikely]]
        return Status::BAD_PAR;

    // Ensure DID has a device table entry
    if (!init_table (baser_dev, did)) [[unlikely]]
        return Status::MEM_OBJ;

    if (!Dev::find (id, did)) [[likely]] {

        // Allocate ITT for DEV
        auto const itt { Buddy::alloc (0, Buddy::Fill::BITS0) };

        // ITT allocation failure
        if (!itt) [[unlikely]]
            return Status::MEM_OBJ;

        // Allocate DEV
        auto const dev { new Dev { id, did, itt } };

        // DEV allocation failure
        if (!dev) [[unlikely]]
            return Status::MEM_OBJ;

        trace (TRACE_INTR | TRACE_PARSE, "GITS: %#010lx MAPD DID:%#x => ITT:%#lx ORD:%u", phys, did, dev->get_itt(), ord_itt);

        // Map DEV => ITT:ORD
        if (!command (Cmd_mapd { did, dev->get_itt(), ord_itt })) [[unlikely]]
