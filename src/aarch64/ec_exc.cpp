...
void Ec_arch::handle_exc_user (Exc_regs *r)
{
    ...
    if (esr == (VAL_SHIFT (0x15, 26) | BIT (25) | 0)) [[likely]]
        (*syscall[r->sys.gpr[0] & BIT_RANGE (3, 0)])(self);

    // SVC #1 from AArch64 state
    else if (esr == (VAL_SHIFT (0x15, 26) | BIT (25) | 1))
        resolved = self->regs.get_obj() == Pd::root->get_obj() && Cpu::feature (Cpu::Cpu_feature::EL3) && Smc::proxy (r->sys.gpr);
...
void Ec_arch::handle_irq_user()
{
    ...
    Event::Selector const evt { Interrupt::handler (self->is_vcpu()) };
