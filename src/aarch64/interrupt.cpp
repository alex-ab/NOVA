...
Event::Selector Interrupt::handle_ppi (uint32_t val, bool)
{
...
{
    trace (TRACE_INTR, "INTR: Routing SPI %#06x (%c%c%c) to CPU %u", spi, cfg.msk() ? 'M' : 'U', cfg.trg() ? 'L' : 'E', cfg.gst() ? 'G' : 'H', cfg.cpu());
