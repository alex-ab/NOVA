...
uint32_t Cpu::cstates, Cpu::topology, Cpu::features[13], Cpu::upatch;
...
void Cpu::enumerate_features (uint32_t &clk, uint32_t &rat, uint32_t (&lvl)[4], uint32_t (&name)[12])
{
    ...
    if (vendor == Vendor::INTEL) {
        ...
        platform = static_cast<uint8_t>(Msr::read (Msr::Reg64::IA32_PLATFORM_ID) >> 50) & BIT_RANGE (2, 0);
    }
    ...
    switch (static_cast<uint8_t>(eax)) {
        ...
        case 0x1a ... 0x1e:
            ...
            type = static_cast<uint8_t>(eax >> 24);
            [[fallthrough]];
        case 0x15 ... 0x19:
    ...
    upatch = static_cast<uint32_t>(Msr::read (Msr::Reg64::IA32_BIOS_SIGN_ID) >> 32);
...
void Cpu::setup_cst()
{
    ...
    trace (TRACE_PWR, "CCST:%s%s%s%s%s%s%s (%#llx:%#llx)",
...
void Cpu::setup_pst()
{
    ...
    if (feature (Feature::HWP)) {
        ...
        trace (TRACE_PWR, "CPST: %u-%u-%u using %u (HWP)", max, gtd, min, des);
        ...
        trace (TRACE_PWR, "CPST: %u-%u-%u using %u (EIST)", max, gtd, min, des);
    }
}
...
void Cpu::init()
{
    ...
    auto const t { type == 0x40 ? "P " : type == 0x20 ? "E " : type ? "? " : "" };

    trace (TRACE_CPU, "CORE: %02u:%02u.%u %s%x:%x:%x:%x [%x] %.48s", lvl[2], lvl[1], lvl[0], t, family, model, stepping, platform, upatch, reinterpret_cast<char *>(name));
