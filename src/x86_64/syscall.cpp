...
void Ec::send_msg()
{
    ...
    if (!obj || !obj->istype (Kobject::Type::PT)) [[unlikely]]
        die ("PT not found");
...
void Ec::sys_create_pd()
{
    ...
    if (!cap.obj() || !cap.obj()->istype (Kobject::Type::PD) || !(cap.prm() & BIT (std::to_underlying (Kobject::Type::PD)))) [[unlikely]] {
...
void Ec::sys_create_ec()
{
    ...
    Capability cap = Space_obj::lookup (r->pd());
    if (!cap.obj() || !cap.obj()->istype (Kobject::Type::PD) || !(cap.prm() & BIT (std::to_underlying (Kobject::Type::EC)))) [[unlikely]] {
        trace (TRACE_ERROR, "%s: Non-PD CAP (%#lx)", __func__, r->pd());
        sys_finish<Sys_regs::BAD_CAP>();
    }
...
void Ec::sys_create_sc()
{
    ...
    Capability cap = Space_obj::lookup (r->pd());
    if (!cap.obj() || !cap.obj()->istype (Kobject::Type::PD) || !(cap.prm() & BIT (std::to_underlying (Kobject::Type::SC)))) [[unlikely]] {
        trace (TRACE_ERROR, "%s: Non-PD CAP (%#lx)", __func__, r->pd());
        sys_finish<Sys_regs::BAD_CAP>();
    }

    cap = Space_obj::lookup (r->ec());
    if (!cap.obj() || !cap.obj()->istype (Kobject::Type::EC) || !(cap.prm() & BIT (std::to_underlying (Kobject::Type::SC)))) [[unlikely]] {
...
void Ec::sys_create_pt()
{
    ...
    Capability cap = Space_obj::lookup (r->pd());
    if (!cap.obj() || !cap.obj()->istype (Kobject::Type::PD) || !(cap.prm() & BIT (std::to_underlying (Kobject::Type::PT)))) [[unlikely]] {
        trace (TRACE_ERROR, "%s: Non-PD CAP (%#lx)", __func__, r->pd());
        sys_finish<Sys_regs::BAD_CAP>();
    }

    cap = Space_obj::lookup (r->ec());
    if (!cap.obj() || !cap.obj()->istype (Kobject::Type::EC) || !(cap.prm() & BIT (std::to_underlying (Kobject::Type::PT)))) [[unlikely]] {
        trace (TRACE_ERROR, "%s: Non-EC CAP (%#lx)", __func__, r->ec());
        sys_finish<Sys_regs::BAD_CAP>();
    }
    Ec *ec = static_cast<Ec *>(cap.obj());
...
void Ec::sys_create_sm()
{
    ...
    Capability cap = Space_obj::lookup (r->pd());
    if (!cap.obj() || !cap.obj()->istype (Kobject::Type::PD) || !(cap.prm() & BIT (std::to_underlying (Kobject::Type::SM)))) [[unlikely]] {
        trace (TRACE_ERROR, "%s: Non-PD CAP (%#lx)", __func__, r->pd());
        sys_finish<Sys_regs::BAD_CAP>();
    }
...
void Ec::sys_ec_ctrl()
{
    ...
    if (!cap.obj() || !cap.obj()->istype (Kobject::Type::EC) || !(cap.prm() & BIT (0))) [[unlikely]] {
...
void Ec::sys_sc_ctrl()
{
    ...
    if (!cap.obj() || !cap.obj()->istype (Kobject::Type::SC) || !(cap.prm() & BIT (0))) [[unlikely]] {
...
void Ec::sys_pt_ctrl()
{
    ...
    if (!cap.obj() || !cap.obj()->istype (Kobject::Type::PT) || !(cap.prm() & BIT (0))) [[unlikely]] {
...
void Ec::sys_sm_ctrl()
{
    ...
    if (!cap.obj() || !cap.obj()->istype (Kobject::Type::SM) || !(cap.prm() & BIT (r->op()))) [[unlikely]] {
...
void Ec::sys_assign_pci()
{
    ...
    if (!obj || !obj->istype(Kobject::Type::PD)) [[unlikely]] {
        trace (TRACE_ERROR, "%s: Non-PD CAP (%#lx)", __func__, r->pd());
        sys_finish<Sys_regs::BAD_CAP>();
    }
...
void Ec::sys_assign_gsi()
{
    ...
    if (!obj || !obj->istype(Kobject::Type::SM)) [[unlikely]] {
    ...
    if (!Gsi::gsi_table[gsi].ioapic && (!Pd::current->Space_mem::lookup (r->dev(), phys) || ((rid = Pci::phys_to_rid (phys)) == ~0U && (rid = Hpet::phys_to_rid (phys)) == ~0U))) [[unlikely]] {
        trace (TRACE_ERROR, "%s: Non-DEV CAP (%#lx)", __func__, r->dev());
        sys_finish<Sys_regs::BAD_DEV>();
    }
