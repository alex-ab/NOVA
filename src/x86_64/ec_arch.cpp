...
Ec *Ec::create_gst (Status &s, Pd *pd, bool t, bool fpu, cpu_t cpu, unsigned long evt, uintptr_t sp, uintptr_t hva)
{
    ...
    if (has_vmx) {
        ...
        if ((!fpu || f) && v && k && (ec = new (cache) Ec_arch { t, f, ref_obj, ref_hst, v, cpu, evt, sp, hva, k })) [[likely]] {
        ...
        if ((!fpu || f) && v && (ec = new (cache) Ec_arch { t, f, ref_obj, ref_hst, v, cpu, evt, sp })) [[likely]] {
            assert (!ref_obj && !ref_hst);
            return ec;
        }
...
void Ec_arch::ret_user_vmexit_vmx (Ec *const self)
{
    ...
    trace (TRACE_CONT, "EC:%p %s to CS:%#x IP:%#lx", static_cast<void *>(self), __func__, Vmcs::read<uint16_t>(Vmcs::Encoding::GUEST_SEL_CS), Vmcs::read<uintptr_t>(Vmcs::Encoding::GUEST_RIP));
    ...
    if (Cr::get_cr2() != r.exc.cr2) [[unlikely]]
        Cr::set_cr2 (r.exc.cr2);
