...
bool Txt::check_acm (Mle_header *hdr, uint32_t sinit_base, uint32_t sinit_size, uint32_t fms, uint32_t &acm_size, uint32_t &acm_caps, uint32_t &tpm_caps)
{
    ...
    if (EXPECT_FALSE (info->min_ver_mle_hdr > hdr->version))
        return false;
...
void Txt::launch()
{
    ...
    if (EXPECT_FALSE (!check_acm (mle, sinit_base, sinit_size, fms, acm_size, acm_caps, tpm_caps)))
        return;
    ...
    if (EXPECT_FALSE (!init_heap (mle, heap_base, heap_size, acm_caps, tpm_caps, vcnt)))
        return;
    ...
    for (; static_cast<void const *>(e) < l; e = e->get_next()) {
        ...
        switch (e->get_type()) {
            ...
            case Element::Type::CEDT:
                reinterpret_cast<Acpi_table const *>(e->get_data())->validate (e->get_data() - o, true);
                break;
