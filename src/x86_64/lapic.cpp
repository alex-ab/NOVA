...
void Lapic::init (uint32_t clk, uint32_t rat)
{
    ...
    if (!eoi_sup()) [[unlikely]]
        panic ("EOI suppression not supported");

    // SW enable with EOI suppression
    write (Reg32::SVR, read (Reg32::SVR) | BIT (12) | BIT (8));
    ...
    trace (TRACE_INTR, "APIC: LOC:%#04x VER:%#x LVT:%#x (x%sAPIC %s Mode)", id[Cpu::id], version(), lvt_max(), x2apic ? "2" : "", ratio ? "OS" : "DL");
}
