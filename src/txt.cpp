...
{
    auto const acm { static_cast<Acm::Header const *>(Hptp::map (Pd::kern.quota, MMAP_GLB_MAP0, sinit_base)) };
...
void Txt::launch()
{
    ...
    if (!Hptp::map (Pd::kern.quota, MMAP_GLB_TXTC, txt_base, Paging::Permissions (Paging::G | Paging::W | Paging::R), Memattr::dev(), 1))
        return;
    ...
    // Map TXT heap
    if (!Hptp::map (Pd::kern.quota, MMAP_GLB_TXTH, heap_base, Paging::Permissions (Paging::G | Paging::W | Paging::R), Memattr::ram(), 2))
        return;
    ...
    if (error & BIT (31)) [[unlikely]] {
        trace(0, "%s:%u triggered by %s - SW source %s", __func__, __LINE__,
              error & BIT (30) ? "external software" : "by processor",
              error & BIT (15) ? "MLE" : "ACM");

        if (!(error & BIT (30)) || error >> 4 & BIT_RANGE (5, 0)) {
            trace(0, "%s:%u previous TXT launch failed - skip - error=%x", __func__, __LINE__, error);
            return;
        }
    }
    ...
    trace(0, "%s:%u TPM init ...", __func__, __LINE__);

    // Abort if TPM initialization fails
    if (!Tpm::init (false)) [[unlikely]]
        return;

    trace(0, "%s:%u Checking ACM ...", __func__, __LINE__);
    ...
    trace(0, "%s:%u SMX:senter ...", __func__, __LINE__);
...
void Txt::init()
{
    ...
    if (pre_acm->caps & Acm::Cap::WAKEUP_GETSEC)
        ...
        *static_cast<Atomic<uint32_t> *>(Hptp::map (Pd::kern.quota, MMAP_GLB_MAP0, acm_mle->rlp_wakeup, Paging::W, Memattr::ram(), 1)) = 1;
}
