...
class Tpm final : private Tcg
{
    private:
        ...
        [[nodiscard]] static bool v2_shutdown() { return execute (Locality::L0, Tpm2_shutdown { Tpm_su::Type::SU_CLEAR }); }
        [[nodiscard]] static bool v1_shutdown() { return execute (Locality::L0, Tpm1_shutdown {}); }

        [[nodiscard]] static bool v2_pcr_reset (unsigned pcr) { return execute (Locality::L2, Tpm2_pcr_reset { pcr }); }
        [[nodiscard]] static bool v1_pcr_reset (unsigned pcr) { return execute (Locality::L2, Tpm1_pcr_reset { pcr }); }

        template<typename T>
        [[nodiscard]] static bool v2_pcr_extend (unsigned pcr, T             const &hash) { return execute (Locality::L2, Tpm2_pcr_extend { pcr, hash }); }
        [[nodiscard]] static bool v1_pcr_extend (unsigned pcr, Hash_sha1_160 const &hash) { return execute (Locality::L2, Tpm1_pcr_extend { pcr, hash }); }

        static bool v2_cap_pcrs();
        static bool v2_cap_tpm_properties();
        static bool v1_cap_tpm_properties (uint32_t &, Tpm1_ptg::Type);

        static inline uint32_t tpm_mfr { 0 }, num_pcr { 0 }, max_buf { 0 };
