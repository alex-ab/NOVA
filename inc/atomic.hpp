 ...
 */
template<typename T, int L = __ATOMIC_RELAXED, int S = __ATOMIC_RELAXED> class Atomic final
{
    private:
        T val;
    ...
    public:
        ...
        ALWAYS_INLINE inline bool compare_exchange_n (T &o, T  n) { return __atomic_compare_exchange_n (&val, &o,  n, false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST); }
...
/*
 * Atomic Pointer Type T*
 */
template<typename T, int L, int S> class Atomic<T*, L, S> final
{
    private:
        T* ptr;
    ...
    public:
        ...
        {
            T* p;
            __atomic_load (&ptr, &p, m);
            return p;
        }

        ALWAYS_INLINE inline void store (T* p, int m = S)
        {
            __atomic_store (&ptr, &p, m);
        }
