...
class Gicc final : private Intid
{
    ...
    private:
        ...
        static inline uint64_t phys { Board::gic[2].mmio };

        static inline auto read  (Reg32 r)      { return *reinterpret_cast<uint32_t volatile *>(MMAP_GLB_GICC + std::to_underlying (r)); }
        static inline void write (Reg32 r, uint32_t v) { *reinterpret_cast<uint32_t volatile *>(MMAP_GLB_GICC + std::to_underlying (r)) = v; }
        ...
        static inline void send_sgi (uint64_t v)
        {
            /*
